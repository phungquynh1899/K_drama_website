...
/**
 * Send cancel job request to Server B
 * @param {string} uploadId
 * @param {string} userId
 * @param {string} reason
 * @param {number} totalChunks
 * @param {string} error
 * @param {number} [failedChunkIndex] - optional chunk index that failed
 */
async function sendCancelJobRequest(uploadId, userId, reason, totalChunks, error, failedChunkIndex = null) {
    try {
        ...
        const cancelBody = {
            ...
            totalChunks,
            error,
            ...(failedChunkIndex !== null && { failedChunkIndex })
        };
        
        await axios.post(cancelUrl, cancelBody, {
            headers: { 
                'Content-Type': 'application/json',
                'x-api-key': process.env.UPLOAD_SHARE_API_KEY 
            }
        });
        console.log(`Successfully sent cancel job request to Server B for uploadId: ${uploadId}`);
    } catch (cancelError) {
        console.error(`Failed to send cancel job request to Server B:`, cancelError.message);
...
async function sendChunksToServerB(uploadId, userId, serverBChunkUrl, serverBCompleteUrl) {
    try {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            if (!sent) {
                throw new Error(`Failed to send chunk ${chunkIndex} after ${maxAttempts} attempts: ${lastError && lastError.message}`);
            }
        }

        // Query filetype from the database with retry logic
        let video = null;
        const maxVideoAttempts = 5;
        const delayMs = 1000; // 1 second
        for (let attempt = 1; attempt <= maxVideoAttempts; attempt++) {
            try {
                const videoResponse = await axios.get(`${process.env.DB_SERVER_HOST}/getVideoByUploadId`, {
                ...
                if (attempt === maxVideoAttempts) throw err;
            }
            if (attempt < maxVideoAttempts) {
                console.warn(`Video not found for uploadId ${uploadId}, retrying in ${delayMs}ms (attempt ${attempt}/${maxVideoAttempts})`);
                await new Promise(res => setTimeout(res, delayMs));
            }
        }
        if (!video) {
            throw new Error(`No video found for uploadId: ${uploadId} after ${maxVideoAttempts} attempts`);
        }
        const filetype = video.metadata.original_filetype;
        ...
        // Centralized cancel job request
        try {
            await sendCancelJobRequest(
                uploadId,
                userId,
                `sendChunksToServerB failed: ${error.message}`,
                undefined,
                error.stack || error.message
            );
        } catch (cancelError) {
            console.error('Failed to send cancel job request in catch block:', cancelError.message);
        }
        throw error;
    }
}
