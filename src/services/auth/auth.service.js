...
class AuthService {
    ...
    static logout = async ({ user }) => {
        const db = BetterSqliteDatabase.getInstance();
        const res1 = await db.deleteRefreshTokenByUserId(user.id);
        const res2 = await db.deleteKeyPairByUserId(user.id);
        console.log('logout successfully, res1: ' + res1 + ' res2: ' + res2 + ' userId: '+ userId)
        return 'OK';
    }
    ...
    static validateUserForExternalService = async ()=>{
        return `OK`;
    }
}
