...
/**
 * Calculate the actual total size in MB (rounded up) of all chunk files for a given uploadId.
 * Looks in public/uploads/{uploadId}/ and sums the sizes of all files named chunk_{index}.
 * @param {string} uploadId
 * @returns {number} total size in MB (rounded up)
 */
exports.getActualUploadSizeMB = function(uploadId) {
  const uploadDir = path.join(__dirname, '../../public/uploads/tmp', String(uploadId));
  let totalBytes = 0;
...
/**
 * Delete all chunk files and the upload directory for a given uploadId.
 * @param {string} uploadId
 */
exports.deleteUploadChunks = function(uploadId) {
  const path = require('path');
  const fs = require('fs');
  const uploadDir = path.join(__dirname, '../../public/uploads', String(uploadId));
  if (fs.existsSync(uploadDir)) {
