...
class VideoMetadataController {
    ...
    static updateChunks = async (req, res, next) => {
        try {
            const { videoId, chunkList, serverDPath } = req.body;
            const result = await VideoMetadataService.updateChunks({ videoId, chunkList, serverDPath });
